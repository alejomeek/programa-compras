...
def build_purchase_order_pdf(
    ...
    total_units = int(point_items["Cantidad"].sum())
    total_value = float((point_items["Cantidad"] * point_items["Costo unitario"]).sum())
    totals = Table(
        [
...
def _prepare_items(order_items: pd.DataFrame) -> pd.DataFrame:
    if order_items is None or order_items.empty:
        return pd.DataFrame()
    df = order_items.copy()
    df["Cantidad"] = pd.to_numeric(df["Cantidad"], errors="coerce").fillna(0).clip(lower=0).round().astype(int)
