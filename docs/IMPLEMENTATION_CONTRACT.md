...
| --- | --- | --- | --- |
| D1 | **Full Mercado Libre** (§15.1): ¿sus ventas se siguen sumando al CEDI? | **Cerrado en Fase 3** (ver §16): sí, se suma. `engine.recommendation` lee `sales_lines` cruda por `location = FULLML` y la suma a CEDI solo en memoria al calcular; el dato de origen nunca se reasigna. | ~~Fase 3~~ cerrado |
| D2 | **Días objetivo** (§15.2): ¿globales por ubicación, por proveedor o por corrida? | **Cerrado en Fase 3** (ver §16): global por ubicación, editable por corrida, con fotografía guardada (`purchase_run_target_days`). Default: 45 días (`engine.recommendation.DEFAULT_TARGET_DAYS`, heredado de `AnalysisConfig`). | ~~Fase 3~~ cerrado |
| D3 | **Productos nuevos** (§15.3): ¿se agregan manualmente a una orden desde el lanzamiento? ¿requieren aprobación previa? | Sí, agregables manualmente; sin aprobación previa definida. | Fase 4 (afecta si `purchase_order_items` admite filas sin `purchase_run_line_id`) |
| D4 | **Numeración y PDF de OC** (§15.4): formato del consecutivo; ¿requiere IVA/flete/vencimiento/condiciones de pago?; ¿una OC por punto o consolidada por proveedor? | **Cerrado:** una OC por ubicación; consecutivo nuevo `OC-UBICACION-NNNN` (serial global que no se reutiliza); PDF con cantidades, costos y totales solamente, sin IVA/flete/vencimiento/condiciones. Las órdenes históricas preservan su numeración. Cualquier rol autenticado puede cancelar una emitida, con motivo obligatorio. | ~~Fase 4~~ decisiones cerradas |
...
| D6 | **Vigencia de archivos TBC** (§15.6): ¿cuántos días de antigüedad admite una importación para usarse en una corrida? ¿se puede elegir entre varias importaciones históricas? | **Cerrado en Fase 3** (ver §16): sin límite de antigüedad. `GET /api/purchase-runs/new-run-options` lista todas las fuentes `status='active'` (no `superseded`), más recientes primero; el comprador elige. | ~~Fase 3~~ cerrado |
...
Las migraciones 0001-0013 ya están escritas, verificadas contra un contenedor Postgres efímero (`supabase/tests/run_migration_tests.sh`, 139 asertos) y **aplicadas al proyecto Supabase real** — 0001-0012 confirmadas end-to-end en producción durante el hotfix de §15 (un `import_job` real completó 223.780 líneas), y `0013` (retiro de Feria/Bodega Bqlla) aplicada por el usuario directo en el SQL Editor de Supabase.
...
| `purchase_orders` / `purchase_order_items` | select; puede cancelar emitida vía RPC con motivo | select + crear/editar `draft`; puede cancelar emitida vía RPC con motivo | ídem |
...
- **Aplicado al proyecto Supabase real por el usuario**, directo en el SQL Editor (no vía este repo ni vía `supabase` CLI — el CLI nunca llegó a autenticarse contra la cuenta real en esta sesión). `locations.active = false` para Feria/Bodega Bqlla y `inventory_snapshots` sin `fair_mode` ya reflejan la decisión en la base real que usan Preview y Producción.
...
Eso cierra la verificación de la **lógica** de las policies. El camino completo con un JWT emitido de verdad por Supabase Auth (`@supabase/ssr` → Postgres real) se verificó aparte: el usuario inició sesión en Producción con una cuenta real (`marialumeek@gmail.com`, distinta de la que subió las importaciones) y confirmó que `/imports` muestra las mismas importaciones recientes que ve la cuenta admin que las creó — coincide exactamente con `C01 viewer lee import_jobs` de la suite pgTAP: la policy de lectura es `using (current_user_role() is not null)`, **sin** filtrar por `created_by`, por la decisión D5 ("sin restricción por defecto", cerrada en Fase 1, ver §13). Con un JWT real contra el proyecto hosted real, el comportamiento fue exactamente el predicho por la simulación — cierra el riesgo abierto de §14 ("RLS con JWT reales... en Supabase hosted, sin verificar").

## 16. Estado de Fase 3 (compras sugeridas) — completada por el equipo

Motor de recomendación, esquema, API y UI completos. Cierra D1/D2/D6 (§2) con las decisiones que confirmó el usuario. Diseñado investigando el código real (no un resumen de memoria) y validado en dos capas independientes antes de tocar el proyecto Supabase real: Docker efímero para SQL, `FakeConnection`/`pytest` para Python.

### Universo de productos elegibles (decisión de diseño, no de negocio)

`supplier_products` existe en el esquema desde Fase 1 pero ningún pipeline lo escribe (confirmado por grep en todo el repo) — no podía ser la fuente de elegibilidad. La lista de precios vigente del proveedor (`price_list_items`) define por sí sola los EAN comprables de una corrida. `inventory_lines` solo aporta `stock_reference` para esos EAN; ni el comodín TBC ni un producto presente solo en inventario pueden crear una línea de compra. Un EAN ausente de la lista se interpreta como no disponible para comprar, no como `no_price`.

### Motor (`engine/`)

- **`engine/recommendation.py`** (nuevo): `prepare_recommendation(conn, *, supplier_id, sales_import_id, price_list_id, inventory_snapshot_id=None, target_days=None)`. Primer módulo de `engine/` que lee de Postgres en vez de un archivo — hace sus propias queries `SELECT` bulk contra `locations`/`sales_imports`/`suppliers`/`price_lists`/`inventory_lines`/`price_list_items`/`sales_lines`/`products`, sin ORM, con el mismo `conn` inyectado que el resto del motor. Fórmula sin floats (`-(-(sales_units*days)//period_days)`, ceil por división entera negada) para reproducibilidad exacta bit a bit; `params_hash` (sha256 de los parámetros de entrada, `target_days` incluido) permite verificar "mismos parámetros ⇒ mismo resultado" sin comparar todas las líneas.
- **`engine/purchase_runs.py`** (nuevo): `persist_purchase_run(...)`. **No reutiliza** `engine.persistence.ImportTarget`/`persist_import` — esa maquinaria asume un `import_jobs` preexistente y supersesión por período, ninguna de las dos cosas aplica a una corrida calculada (nace y se calcula en el mismo paso, su ciclo de vida posterior es edición vía RPC). Sí reutiliza los helpers DB-API genéricos (`readable_error`, `_rollback`, `_close`, `_placeholders`, importados directo de `engine.persistence`). Una sola transacción: `purchase_runs` (nace `draft`) + `purchase_run_target_days` + `purchase_run_lines`, sube a `calculated`; cualquier excepción hace `rollback` — `status='failed'` en el resultado Python, pero la fila **nunca llega a existir** en la base (a diferencia de un `import_job`, no hay fila previa que marcar `failed`; el enum `purchase_run_status` ni siquiera tiene ese valor).
- Pruebas: `tests/python/test_recommendation.py` (casos de ventas=0, redondeo, período de 1 día, Full ML sumado a CEDI sin fila propia, días objetivo independientes, reproducibilidad, elegibilidad definida exclusivamente por lista de precios, producto nuevo, validaciones, y una prueba estructural anti-regresión en memoria) y `tests/python/test_purchase_runs_persistence.py` (todo-o-nada, conteos). `tests/python/conftest.py`'s `FakeConnection` extendido con kwargs `rec_*` (un escenario por prueba, mismo criterio que `job_status`/`max_version` ya establecidos) y `close()` (lo necesitaba también `test_imports_process.py`).

### Esquema (`supabase/migrations/0014_purchase_runs.sql`)

4 tablas del contrato §6.3 tal cual, sin ambigüedad: `purchase_runs`, `purchase_run_target_days`, `purchase_run_lines` (`suggested_quantity` inmutable por trigger; `unique(purchase_run_id, ean, location_id)` porque `product_id` puede ser nulo), `purchase_line_adjustments` (append-only, trigger `security definer` la inserta, nadie más escribe). RPC `update_final_quantity` (`security definer`, único camino de escritura de `final_quantity`, concurrencia optimista por `row_version` — 0 filas afectadas ⇒ `ROW_VERSION_CONFLICT` explícito).

**Decisión de RLS que se apartó de una lectura literal del contrato**, encontrada al escribir las pruebas de comportamiento (`80_purchase_runs.sql`, caso "admin cancela una corrida bloqueada"): la policy `purchase_runs_update_writer` necesita el mismo bypass de `is_admin()` en `using` que en `with_check` — sin eso, un admin no podría ni empezar a actualizar una fila ya `locked` (`using` decide qué filas son candidatas ANTES de evaluar el nuevo valor), y "admin cancela/elimina" del contrato §7 quedaría sin poder ejercerse sobre una corrida bloqueada. Corregido antes de aplicar la migración en ningún lado.

`purchase_run_lines` **no** tiene policy de `UPDATE` para `authenticated` a propósito: si la tuviera, un PATCH directo por PostgREST saltaría el chequeo de `row_version` del RPC, que es justo lo que protege el contrato §9.

Empuja las migraciones planificadas después una posición (§6.2, ya reflejado en la tabla: `0015_purchase_orders.sql` en adelante, Fase 4).

**Verificado con `supabase/tests/run_migration_tests.sh`** (Docker efímero, nunca contra el proyecto real): 174/174 asertos — 139 de Fase 1/2 sin regresión + 26 nuevos en `80_purchase_runs.sql` (estructura anti-regresión autoritativa vía `information_schema.columns`, RLS por rol, el trigger de inmutabilidad, y el RPC en sus tres caminos: feliz, conflicto de versión, corrida bloqueada) + 9 nuevos en `10_structure_and_grants.sql` (enums, columna generada, grants).

**Pendiente, acción manual fuera de este repo**: aplicar `0014_purchase_runs.sql` al proyecto Supabase real (el mismo procedimiento que usó el usuario para `0013`: SQL Editor).

### API (`api/purchase_runs_calculate.py`, `src/app/api/purchase-runs/**`)

`api/purchase_runs_calculate.py` es mirror de `api/imports_process.py` (misma frontera: única conexión `service_role` real de este flujo) pero más simple — sin `Storage` (las fuentes ya están en la base) y sin lógica de "marcar failed" (no hay fila previa que dejar huérfana, ver arriba). `POST /api/purchase-runs` sigue la arquitectura de dos saltos de Fase 2 (Next.js → función Python con `INTERNAL_API_SECRET` + `x-vercel-protection-bypass` si `VERCEL_AUTOMATION_BYPASS_SECRET` existe, mismo helper que Fase 2), pero a diferencia de `/api/imports` **espera** la respuesta del cálculo — no existe un "quedó en cola" válido para una corrida, el comprador pidió un cálculo, no una carga async.

`POST /api/purchase-runs/[id]/lines/[lineId]/adjust` llama al RPC vía `supabase.rpc(...)` y traduce el error por `error.code` (el SQLSTATE, más robusto que parsear texto) con `mapAdjustError` (`src/app/api/purchase-runs/adjust-error.ts`, probado aparte): `40001`→409 conflicto de versión (relee la línea y devuelve el valor vigente), `P0002`→404, `55000`→409 corrida bloqueada, `42501`→403, `23514`→400.

### UI (`src/app/(app)/purchase-runs/**`, `src/components/purchase-runs/**`)

Los dos stubs de Fase 2 (`page.tsx`, `[id]/page.tsx`, ya en `src/lib/nav.ts`) se completaron sin tocar el nav. Mismo patrón 1:1 que `/imports`: Server Component (fetch + DTO camelCase) + Client Component wrapper solo donde hace falta pasar funciones + módulo de presentación puro (`run-status.ts`, reexporta los formatters genéricos de `job-status.ts` en vez de duplicarlos, con sus propias pruebas). Tabla de líneas paginada del lado del servidor (a diferencia de `/imports`, que nunca paginó: una corrida real puede tener cientos/miles de filas producto×ubicación) con edición inline de `final_quantity` por fila y manejo explícito del 409 (nunca sobrescribe en silencio: acepta el valor vigente que devuelve el servidor). Sin dependencias nuevas (`<Input type="number">` a mano dentro de `<Table>`, confirmado suficiente a esta escala).

### Verificado, con matices

- `npm run typecheck`, `npm run lint`, `npm run test` (105 pruebas vitest, incluidas `run-status.test.ts`, `calculate-request.test.ts`, `adjust-error.test.ts`) y `pytest tests/python` (282 pruebas) — todos limpios.
- `npm run dev` + `curl` contra las rutas nuevas sin sesión: `/purchase-runs` y `/purchase-runs/[id]` responden 307 (redirect a `/login`, esperado), `POST /api/purchase-runs` responde 401 (`requireWriter()`, esperado) — confirma que compilan y corren sin errores de build/runtime, pero **no** es un smoke test autenticado.
- **No verificado**: flujo completo en el navegador con un usuario real logueado (crear una corrida, ver sus líneas, ajustar una cantidad, provocar un conflicto de versión) — la extensión de Chrome no estaba conectada en esta sesión. Pendiente antes de considerar Fase 3 probada de punta a punta, mismo criterio que cerró Fase 2.
