...
`supplier_products` existe en el esquema desde Fase 1 pero ningún pipeline lo escribe (confirmado por grep en todo el repo) — no podía ser la fuente de elegibilidad. `engine.recommendation` usa en su lugar la unión de: (a) EAN con `supplier_tbc_code` del proveedor en `inventory_lines` del snapshot elegido, y (b) EAN en `price_list_items` de la lista elegida — mismas dos fuentes que ya usaba el motor MVP viejo. Un elegible sin precio vigente se guarda con `suggested_quantity = 0` y `status = 'no_price'` (no se descarta), para que el comprador lo agregue a mano.
...
- Pruebas: `tests/python/test_recommendation.py` (22 casos: ventas=0, redondeo, período de 1 día, Full ML sumado a CEDI sin fila propia, días objetivo independientes, reproducibilidad, elegibilidad con/sin precio, producto nuevo, validaciones, y una prueba estructural anti-regresión en memoria) y `tests/python/test_purchase_runs_persistence.py` (todo-o-nada, conteos). `tests/python/conftest.py`'s `FakeConnection` extendido con kwargs `rec_*` (un escenario por prueba, mismo criterio que `job_status`/`max_version` ya establecidos) y `close()` (lo necesitaba también `test_imports_process.py`).
